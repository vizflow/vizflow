var jarHelper = {
  ...
  y_trans: function jar_helper_y_trans(viz, code) {
    ...
    return transitionHelper.new_linear('y', yNew, viz.duration) ;
  ...
  x_trans: function jar_helper_x_trans(viz, code) {
    ...
    return transitionHelper.new_linear('x', xNew, viz.duration) ;
  ...
  click: function jar_helper_click( jar ) {
    ...
    viz.add_sequence( [true, false], transitionHelper.fixed_duration_step('busy', viz.duration * 6) ) ;
    ...
    if ( jar.all_collected() ) {
      ...
      for ( var kitem = 0 ; kitem < jar.fruit.item.length ; kitem++ ) {
        ...
        var xTrans0 = transitionHelper.new_linear('x', jar.x0 + shift, viz.duration * 3) ; 
        xTrans0.child = xTrans ;

        var yTrans0 = transitionHelper.new_linear('y', jar.y0, viz.duration * 3) ; 
  ...
  show_prime: function jar_helper_show_prime( jar ) {
    ...
    function x_scale(shift) {
      ...
      var xScaleTrans   = transitionHelper.new_linear('xScale', scale0, viz.duration) ;
      xScaleTrans.pause = viz.duration * 3 ;
      xScaleTrans.child = transitionHelper.new_linear('xScale', scale1, viz.duration) ;
      return xScaleTrans ;
    ...
    function y_scale(shift) {
      ...
      var yScaleTrans   = transitionHelper.new_linear('yScale', scale0, viz.duration) ;
      yScaleTrans.pause = viz.duration * 3 ;
      yScaleTrans.child = transitionHelper.new_linear('yScale', scale1, viz.duration) ;     
      return yScaleTrans ;
    ...
    var xTrans0 = transitionHelper.new_linear('x', jar.x0, viz.duration * 4) ;
    var yTrans0 = transitionHelper.new_linear('y', jar.y0, viz.duration * 4) ;
